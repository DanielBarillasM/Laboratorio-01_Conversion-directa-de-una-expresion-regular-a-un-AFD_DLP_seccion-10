...
- Reinicia VSCode (si ejecutas desde allí).
- Verifica el PATH:
...
- http://localhost:3000/render?re=(a|b)*abb

### SVG directo del AFD
- http://localhost:3000/dfa.svg?re=(a|b)*abb
...
- Si `STRICT_OPERATORS_ONLY` está en `true` (por defecto), se rechazan operadores estilo RegExp: `+ ? { } [ ] . ^ $` si no están escapados.
...
- La lógica de compiladores se implementa desde cero:
  - tokenización, shunting-yard, árbol, followpos, construcción de DFA
